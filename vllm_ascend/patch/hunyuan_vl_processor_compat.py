...
def _uses_legacy_bundled_processors() -> bool:
    ...
    return all(bundled_modules.values())


def _import_legacy_hunyuan_vision() -> Any:
    """Import a legacy model with the native processors from vLLM PR #47872."""
...
def _patch_legacy_processor_methods(hunyuan_vision: Any) -> None:
...
def install_hunyuan_vl_processor_compat() -> None:
    ...
    if _uses_legacy_bundled_processors():
        legacy_hunyuan_vision = _import_legacy_hunyuan_vision()
        _remove_stale_registry_entries()
        _patch_hunyuan_processor_loader(legacy_hunyuan_vision)
        _patch_legacy_processor_methods(legacy_hunyuan_vision)
        return
