...
class AscendQwen3NextAttention(Qwen3NextAttention):
    def _forward_ascend(self, positions: torch.Tensor, hidden_states: torch.Tensor) -> torch.Tensor:
    ...
    if vllm_version_is("0.23.0"):
        ...
        def forward(
            ...
            output[:] = self._forward_ascend(positions, hidden_states)

    else:

        def forward(self, positions: torch.Tensor, hidden_states: torch.Tensor) -> torch.Tensor:
            return self._forward_ascend(positions, hidden_states)
