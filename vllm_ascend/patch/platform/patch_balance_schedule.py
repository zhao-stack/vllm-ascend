...
``Scheduler.schedule()`` (the production pin), plus six maintained deltas:
(1) the disabled-path early return that delegates to ``super()``, (2) the
global balance break inside the WAITING loop (``any-rank-at-cap => global
freeze``), (3) ``if request_queue is None: break`` in place of upstream's
assertion for a drained rank, and three vLLM #47782 protocol bridges: (4) the
v0.24 pair / main triple ``get_computed_blocks`` unpack, (5) resetting the
main-only shared-prefix boundary after the v0.24 per-group hybrid lookup, and
(6) the v0.24 five-argument / main four-argument Mamba split call.

Both supported vLLM lanes accept ``schedule(throttle_prefills=False)``, so
the override forwards that argument directly on its disabled path. The body
tracks the stable release tag; the three protocol bridges keep the same copy
compatible with the verified main commit.
                ...
                if request.num_computed_tokens == 0:
                    ...
                    if (
                        ...
                        if not vllm_version_is("0.24.0"):
                            # Per-group lookup has no shared-prefix junction.
